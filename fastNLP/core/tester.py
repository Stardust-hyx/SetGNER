...
from .sampler import Sampler, ConstTokenNumSampler
...
class Tester(object):
    ...
    def __init__(self, data, model, metrics, batch_size=16, num_workers=0, device=None, verbose=1, use_tqdm=True,
        ...
        if isinstance(data, DataSet):
            ...
            if isinstance(sampler, ConstTokenNumSampler):
                ...
                sampler(data)
                self.data_iterator = DataSetIter(data, batch_size=1, sampler=None, as_numpy=False, num_workers=num_workers, batch_sampler=sampler)
                check_code_level = -1  # 强制跳过校验
            else:
                self.data_iterator = DataSetIter(dataset=data, batch_size=batch_size, sampler=sampler, num_workers=num_workers)
    ...
    def _format_eval_results(self, results):
        ...
        for metric_name, metric_result in results.items():
            _str += metric_name + ': '
            _str += ", ".join([str(key) + "=" + str(value) for key, value in metric_result.items() if not isinstance(value, (list, dict))])
            _str += '\n'
        return _str[:-1]
