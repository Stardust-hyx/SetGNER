    ...
    def _do_validation(self, epoch, step):
        ...
        if self._better_eval_result(res):
            if self.save_path is not None:
                # self._save_model(self.model, "best_" + "_".join([self.model.__class__.__name__, self.metric_key, self.start_time]))
                self._save_model(self.model.seq2seq_model.encoder.bart_encoder,
                                 "best_" + "_".join([self.model.__class__.__name__, 'encoder', self.metric_key, self.start_time]))
                self._save_model(self.model.seq2seq_model.decoder.decoder,
                                 "best_" + "_".join([self.model.__class__.__name__, 'decoder', self.metric_key, self.start_time]))
